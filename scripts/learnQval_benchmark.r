...
##   SIMULATE DRAFTS AND EVALUATE POLICIES WITH DIFFERENT Q-FUNCTIONS

##   confirm within year performance
results_all= c()

for (trainTarg in c('a')){
  for (curyear in c(2014,2015,2016)){
    for (mtype in c('xgboost', 'glmnet')){
      model= list('WR'=NULL,'RB'=NULL,'TE'=NULL,'QB'=NULL)
      
      ##   logged exploration data from 5000 rounds of eps-greedy with xgboost
      options_log= fread(paste0('./optionslog', curyear, trainTarg, '.csv'))
      results= fread(paste0('./results', curyear, trainTarg, '.csv'))
      
      ## select a q-function
      model= updateQ(model, iters= 30, mtype= mtype, gamma=.9)
      ...
      for (trial in 1:500){
        ...
        pod[,c('round', 'val_rank', 
               'exp_rank', 'b', 
               'baselineAlgo', 'model', 
               'testSet', 'trainTarg'):=list(trial, 
                                                val_rank, 
                                                exp_rank, 
                                                ceiling(trial/50), 
        ...
        results_all= rbind(results_all, pod[fteam==vbdTurn])
  ...
  ##   evaluate cross year performance
  for (curyear in c(2014,2015,2016)){
    for (mtype in c('xgboost', 'glmnet')){
      model= list('WR'=NULL,'RB'=NULL,'TE'=NULL,'QB'=NULL)
      
      ##   logged exploration data from 5000 rounds of eps-greedy with xgboost
      options_log= rbind(fread(paste0('./optionslog', 2014, trainTarg, '.csv')), 
                         fread(paste0('./optionslog', 2015, trainTarg, '.csv')), 
                         fread(paste0('./optionslog', 2016, trainTarg, '.csv')))[year!=curyear]
      
      train_results= rbind(fread(paste0('./results', 2014, trainTarg, '.csv')), 
                           fread(paste0('./results', 2015, trainTarg, '.csv')), 
                           fread(paste0('./results', 2016, trainTarg, '.csv')))[year!=curyear]
...
write.csv(results_all, 
          file='./results_bothModels_withinAndAcrossYears.csv', 
          row.names = F)
